...
* @details This code explains how to configure a receiver for basic packets with payload longer than 96 bytes. The interrupt 
*       of AE is time critical because it is needed to be processed before the FIFO is overflowed.
*       It is safer to manage it on a separate pin.        
...
*/
uint16_t nRxIndex, nPayloadLength = 60;
...
/**
* @brief  This function handles the FIFO Almost Full interrupt request. In thi application it 
          is configured to be notified on the Spirit GPIO_2.
* @param  None
* @retval None
*/
#ifdef STM8L
INTERRUPT_HANDLER(M2S_GPIO_2_EXTI_IRQ_HANDLER,15)
#elif SDK
...
void main (void)
{
...
#endif
  /* configure the EXTI to raise an interrupt on the falling edge of GPIO3 */
  ...
  /* configure the EXTI to raise an interrupt on the rising edge of GPIO2 ... */
  SdkEvalM2SGpioInit(M2S_GPIO_2,M2S_MODE_EXTI_IN);
  
#ifdef STM8L
  EXTI_SetPinSensitivity(M2S_GPIO_2_EXTI_PIN_SOURCE, EXTI_Trigger_Rising);
  enableInterrupts();
#elif SDK
  /* ... since the SdkEvalM2SGpioInit configures the EXTI to raise the IRQ on the falling edge
      we reconfigure the trigger registers */
  SdkEvalM2SGpioTriggerRising(M2S_GPIO_2, ENABLE);
  SdkEvalM2SGpioTriggerFalling(M2S_GPIO_2, DISABLE);
  SdkEvalM2SGpioInterruptCmd(M2S_GPIO_3,0x0A,0x00,ENABLE);
  SdkEvalM2SGpioInterruptCmd(M2S_GPIO_2,0x09,0x00,ENABLE);
#endif
