...
#define ENABLE_TCXO()           {RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOC,ENABLE);\
                                GPIO_Init(GPIOC,&(GPIO_InitTypeDef){GPIO_Pin_2,GPIO_Mode_OUT,GPIO_Speed_40MHz,GPIO_OType_PP,GPIO_PuPd_NOPULL});\
...
void SpiritManagementIdentificationRFBoard(void)
{
  ...
  else  // EEPROM found
  {
    ...
    default:
      {
        uint32_t xtal_freq=0xffffffff;
        if(memcmp(&tmpBuffer[27],(uint8_t*)&xtal_freq,4)==0)
        {
          SpiritManagementComputeXtalFrequency();
        }
        else
        {
          ...
          {
            ((uint8_t*)&xtal_freq)[3-i]=tmpBuffer[27+i];
          }
          SpiritRadioSetXtalFrequency(xtal_freq);
        }
      }
      break;
    }
