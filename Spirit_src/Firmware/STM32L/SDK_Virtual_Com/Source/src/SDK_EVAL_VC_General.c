...
#define USB_ENTER_CRITICAL()              {NVIC_InitTypeDef NVIC_InitStructure = {USB_LP_IRQn, VC_PREHEMPTION_PRIORITY, 0,  DISABLE}; \
  NVIC_Init(&NVIC_InitStructure);} \
    
#define USB_EXIT_CRITICAL()              {NVIC_InitTypeDef NVIC_InitStructure = {USB_LP_IRQn, VC_PREHEMPTION_PRIORITY, 0,  ENABLE}; \
  NVIC_Init(&NVIC_InitStructure);} \
...
int (*SdkEvalVCRxByteCallback)(long nbytes)=NULL;
...
void SdkEvalVCInit(void)
{
  ...
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = VC_PREHEMPTION_PRIORITY;
...
* @param  lInIndex input index.
* @param  lOutIndex output index.
* @retval None.
*/
...
/**
* @brief  Enqueues the rx chars in a circular buffer.
* @param  buffer input in which the rx bytes are stored index.
* @param  size size of the input buffer.
* @note   If the pointer to the callback <i>@ref SdkEvalVCRxByteCallback</i> 
*         has been set through the SdkEvalVCSetRxByteCb function, this function 
*         calls it every time a byte incomes.
* @retval None.
*/
void enqueueRxChars(unsigned char * buffer, uint16_t size)
{
  while (( size > 0 ) && (rxUsed < (RECEIVE_QUEUE_SIZE-1)))
  {
    ...
    if(SdkEvalVCRxByteCallback)
    {
      SdkEvalVCRxByteCallback(rxUsed);
    }
...
/**
* @brief  To set a callback that is called every time a byte is received.
* @param  input pointer to the function to be called.
* @retval None.
*/
void SdkEvalVCSetRxByteCb(int (*input) (long nbytes))
{
  SdkEvalVCRxByteCallback = input;
}
